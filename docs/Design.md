...
纸页：最大宽度 840px，短页最小高度 360px，圆角 4px（纸，不是胖卡片），1px `rule`，阴影用两层（贴桌的 1px 边 + 16px 落影）。内边距 32×24。生成页画在这张纸上，不要再套工坊底。
...
左：印在字前。印是 5×18 圆角 1px 的竖章，`seal` 色，`aria-hidden`。字标「言灵」是 `<h1>`，18px / 600 / 衬线 / 字距 0.16em，`translate="no"`。不要写成 Logo 图。
